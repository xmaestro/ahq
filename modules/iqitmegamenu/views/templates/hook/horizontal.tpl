...
*}
	...
	{if $tab.url_type == 2}<a role="button" class="cbp-empty-mlink nav-link">{else}<a href="{$tab.url}" class="nav-link" {if $tab.new_window}target="_blank"{/if}>{/if}
									...
									<div class="col-2">
										<ul class="cbp-hrsub-tabs-names cbp-tabs-names" >
											...
											<li class="innertab-{$innertab->id} ">
												<a data-target="#{$innertab->id}-innertab-{$tab.id_tab}" {if $innertab->url_type != 2} href="{$innertab->url}" {/if} class="nav-link {if $smarty.foreach.innertabsnames.first}active{/if}">
